...
function handlers()
{
        this.lessCompile = function (file, path, index, isLast, callback) {
            console.log(path);
            if (path.search(/\.css/) != -1) {
                console.log('skipping for CSS file: ' + path);
                callback(file); // if file pure css, skip!
            } else {
                var parser = new(less.Parser)({
                    paths: ['public/css/'], // Specify search paths for @import directives
                    filename: 'bootstrap.less' // Specify a filename, for better error messages
                });

                parser.parse(file, function(e, tree) {
                  if (e) { console.log('LESS PARSE error is: '); /*console.log(e)*/ }
                  callback(tree.toCSS());
                });
            }
        }
